...
export const postRoomRequest = async (
  data: IPostRoomRequest
): Promise<Partial<IRoomResponse>> => {
  const response: AxiosResponse<Partial<IRoomResponse>> = await getApi().post(
    '/rooms',
    data
  );
...
export const addUsersToRoomRequest = async (
  data: IUserIds,
  roomId: string
): Promise<void> => {
...
export const getRoomsRequest = async (): Promise<IGetRoomsResponse> => {
  const response: AxiosResponse<IGetRoomsResponse> = await getApi().get(
    '/rooms'
  );
  return response.data;
};

export const getRoomDetailsRequest = async (
  roomId: string
): Promise<Partial<IRoomResponse>> => {
  const response: AxiosResponse<Partial<IRoomResponse>> = await getApi().get(
    `/rooms/${roomId}`
  );
  return response.data;
};
